...
- **Event-keyed Hooks** — Agent execution on `on_completed`, `on_failed`, `on_blocked`, and `on_spec_completed` (e.g. documentation maintenance fires once per completion). Several hooks can serve one event in order; hooks are not retroactive and never change the loop's status.
